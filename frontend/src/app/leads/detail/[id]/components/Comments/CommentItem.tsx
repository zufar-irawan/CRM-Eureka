...
export default function CommentItem({
  ...
  const renderComment = (comment: Comment, currentDepth: number = 0): JSX.Element => (
    <div key={comment.id} className={`space-y-4 ${currentDepth > 0 ? `ml-${Math.min(currentDepth * 8, 32)}` : ''}`}>
      {/* Comment */}
      ...
      {comment.replies && comment.replies.length > 0 && (
        <div className="space-y-4">
          {comment.replies.map(reply => renderComment(reply, currentDepth + 1))}
        </div>
      )}
    </div>
  );
