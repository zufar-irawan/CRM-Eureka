...
export default function MainLeads() {
  ...
  const fetchLeads = async () => {
    try {
      ...
      let errorMessage = "Terjadi kesalahan saat mengambil data leads";
      ...
      if (error.code === 'ECONNABORTED') {
        ...
        // Request was made but no response received
        errorMessage = "Tidak dapat terhubung ke server. Pastikan server berjalan di localhost:5000";
      } else {
      ...
      <div className="max-w-7xl mx-auto">
              ...
              {loading ? (
                ...
                leads.map((lead) => (
                  <tr key={lead.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-xs">{lead.stage || '-'}</td>
                    <td className="px-6 py-4 text-xs text-gray-900">{lead.email || '-'}</td>
                    <td className="px-6 py-4 text-xs text-gray-900">{lead.mobile || '-'}</td>
                    <td className="px-6 py-4 text-xs text-gray-500">{lead.updated_at || '-'}</td>
        ...
        <div className="lg:hidden space-y-4">
          {loading ? (
            ...
            <div className="text-center py-8">
              <p className="text-gray-500">Tidak ada data leads ditemukan.</p>
            </div>
          ) : (
            leads.map((lead) => (
